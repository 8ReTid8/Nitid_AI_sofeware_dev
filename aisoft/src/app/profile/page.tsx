...
export default function ProfilePage() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch("/api/profile", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ user_name: user.user_name }), // Only send the user_name
            });
            if (!response.ok) throw new Error("Failed to update profile");
            setSuccessMessage("Profile updated successfully!");
            setIsEditing(false);
    ...
    if (!user.user_name) return <div className="flex justify-center items-center h-screen">Loading...</div>;
        ...
        <div className="container mx-auto px-4 py-8 max-w-lg">
            <div className="card bg-base-100 shadow-xl p-6">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="form-control mb-4">
                        <label className="label">Email</label>
                        <input type="email" name="user_email" value={user.user_email} disabled className="input input-bordered" />
                    </div>
                    <div className="flex justify-end mt-4">
                        {!isEditing ? (
                            <button type="button" onClick={() => setIsEditing(true)} className="btn btn-primary">Edit Name</button>
                        ) : (
                            <>
                                <button type="button" onClick={() => setIsEditing(false)} className="btn btn-secondary mr-2">Cancel</button>
                                <button type="submit" className="btn btn-success">Save</button>
